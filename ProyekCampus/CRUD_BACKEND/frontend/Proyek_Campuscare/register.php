...
// Asumsi: Variabel koneksi adalah $koneksi
$db_koneksi = isset($koneksi) ? $koneksi : null;

if (!$db_koneksi) {
    die("Error: Objek koneksi database tidak ditemukan.");
}

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Sanitisasi input
    $nama = mysqli_real_escape_string($db_koneksi, $_POST['nama_lengkap']);
    $nim = mysqli_real_escape_string($db_koneksi, $_POST['nim']);
    $email = mysqli_real_escape_string($db_koneksi, $_POST['email']);
    $password = password_hash($_POST['password'], PASSWORD_DEFAULT); // 🔐 hash password
    $prodi = mysqli_real_escape_string($db_koneksi, $_POST['prodi']);
    
    // Status awal harus 'pending'
    $status_default = 'pending'; 

    // 🔎 cek apakah NIM atau Email sudah terdaftar
    $cek = mysqli_query($db_koneksi, "SELECT * FROM mahasiswa WHERE nim='$nim' OR email='$email'");
    if (mysqli_num_rows($cek) > 0) {
        echo "<script>alert('NIM atau Email sudah terdaftar!'); window.location='register.php';</script>";
        exit();
    }

    // Ubah Query: Tambahkan kolom 'status'
    $query = "INSERT INTO mahasiswa (nama_lengkap, nim, email, password, prodi, status)
              VALUES ('$nama', '$nim', '$email', '$password', '$prodi', '$status_default')";

    if (mysqli_query($db_koneksi, $query)) {
        // Pesan disesuaikan, memberitahu user harus menunggu konfirmasi
        echo "<script>alert('Registrasi berhasil! Akun Anda sedang menunggu konfirmasi admin sebelum bisa login.'); window.location='login.php';</script>";
    } else {
        echo "<script>alert('Terjadi kesalahan: " . mysqli_error($db_koneksi) . "');</script>";
    }
}
?>
